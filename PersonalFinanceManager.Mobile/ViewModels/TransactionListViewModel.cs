...
public class TransactionListViewModel : ViewModelBase
{
    ...
    /// <summary>
    /// Picker-friendly wrapper around NewCategoryId — MAUI's Picker binds SelectedItem
    /// to the object itself, not an id, so this exposes/accepts a CategoryDto.
    /// </summary>
    public CategoryDto? SelectedCategory
    {
        get => Categories.FirstOrDefault(c => c.Id == NewCategoryId);
        set => NewCategoryId = value?.Id ?? default;
    }
    ...
    public async Task LoadAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            // Foreground sync attempt — compensates for Android suspending the
            // background poll loop while the app isn't in use (see SyncService).
    ...
    /// <summary>Manual "pull to refresh" sync trigger (RefreshView binds to this).</summary>
    private async Task SyncNowAsync() => await LoadAsync();
